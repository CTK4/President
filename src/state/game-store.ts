...
import {
  ...
  createNewGame,
  resolveAgendaAction as engineResolveAgendaAction,
  resolveBillAction as engineResolveBillAction,
  resolveCovertOperation as engineResolveCovertOperation,
  resolveElection as engineResolveElection,
  resolveMilitaryAction as engineResolveMilitaryAction,
  resolveResponse as engineResolveResponse,
} from "@/sim/engine";
import { clearSave, loadGame, saveGame } from "@/sim/save/sqlite";
import type { AgendaActionInput, BillAction, CovertOperationInput, CourtNominationStrategy, ElectionType, GameState, MilitaryActionInput, NewGameConfig, PlayerResponse, TurnResolution } from "@/sim/types";
...
export async function submitAgendaAction(input: AgendaActionInput) {
  if (!snapshot.game) return null;
  const resolution = engineResolveAgendaAction(snapshot.game, input);
  await saveGame(resolution.game);
  setSnapshot({ game: resolution.game, lastResolution: resolution });
  return resolution;
}

export async function submitCovertOperation(input: CovertOperationInput) {
  if (!snapshot.game) return null;
  const resolution = engineResolveCovertOperation(snapshot.game, input);
  await saveGame(resolution.game);
  setSnapshot({ game: resolution.game, lastResolution: resolution });
  return resolution;
}

export async function submitMilitaryAction(input: MilitaryActionInput) {
  if (!snapshot.game) return null;
  const resolution = engineResolveMilitaryAction(snapshot.game, input);
  await saveGame(resolution.game);
  setSnapshot({ game: resolution.game, lastResolution: resolution });
  return resolution;
}
